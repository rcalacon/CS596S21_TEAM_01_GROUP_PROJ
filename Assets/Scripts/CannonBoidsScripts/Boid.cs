...
public class Boid : MonoBehaviour
{
    ...
    public void UpdateNeighbors(List<Transform> boids, float distance)
    {
        ...
        {
            var neighbor = boids[neighborIndex];
    ...
    public Vector3 Cohesion(float cohesionSteps, float weight)
    {
        ...
        for (var neighborIndex = 0; neighborIndex < Neighbors.Count; neighborIndex++)
        {
            ...
            var neighborComponent = neighbor.GetComponent<Boid>();
            if (perceivedCenter == Vector3.zero)
            {
                perceivedCenter = neighborComponent.Position;
            }
            else
            {
                perceivedCenter = perceivedCenter + neighborComponent.Position;
            }
        }
    ...
    public Vector3 Separation(float weight)
    {
        ...
        for (var i = 0; i < Neighbors.Count; ++i)
        {
            ...
            var neighborComponent = neighbor.GetComponent<Boid>();
            var distance = Vector3.Distance(Position, neighborComponent.Position);

            c = c + Vector3.Normalize(Position - neighborComponent.Position) / Mathf.Pow(distance, 2);
        }
        return c * weight;
    }
    ...
    public Vector3 Alignment(float weight)
    {
        ...
        for (var neighborIndex = 0; neighborIndex < Neighbors.Count; neighborIndex++)
        {
            ...
            var neighborComponent = neighbor.GetComponent<Boid>();
            pv = pv + neighborComponent.Velocity;
        }
    ...
    public Vector3 Socialize(List<Transform> boids)
    {
        ...
        for (var i = 0; i < boids.Count; ++i)
        {
            ...
            if (Position != boidComponent.Position)
            {
                if (pc == Vector3.zero)
                {
                    pc = boidComponent.Position;
                }
                else
                {
                    pc = pc + boidComponent.Position;
                }
            }
        }
        if (boids.Count > 1)
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        {
            gameManager.GetComponent<CannonBoidsManager>().updateScore();
            //updateScore();
